...
class PagesController extends AppController {
	...
	public function safety() {
		if (!empty($this->request->data['assessment']) and isset($_POST['g-recaptcha-response'])) {
			...
			$secretKey = "6LfOBRETAAAAALco46TJjbp1qfIny7pBe6O0ZlQL";
			...
			$Email->subject('ErgoPlus - Safety Survey Completed - ' . date('m/d/Y h:i:s'));
			...
			if(strpos($request, 'true')){
				...
				$this->Session->setFlash('Please complete the recaptcha to submit your answers.','alert', array('plugin' => 'BoostCake', 'class'=>'alert alert-danger'));
			}else{
	...
	public function contact() {
		...
		if (!empty($this->request->data['contact']) and isset($_POST['g-recaptcha-response'])) {
			...
			$Email->to('troymarshpt@gmail.com');
			$Email->subject('ErgoPlus - Contact Request! - ' . date('m/d/Y h:i:s'));
			$Email->viewVars(array('content' => $this->request->data));
			if(strpos($request, 'true')){
				$Email->send();
